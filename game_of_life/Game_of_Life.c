...
int setup_phase(char** localw, char** local_neww, int N, int size, int global_rank, MPI_Comm* phase_comm){
	char* local = *localw;
	...
	MPI_Comm_split(MPI_COMM_WORLD, color, global_rank, phase_comm);
...
int main(int argc, char *argv[])
{
	...
	//determine number and world_size of each phase. 
	for(int i=0; i<num_phases; i++){
		if(phases[i] > size){printf("PHASE TOO BIG\n");}
	} //stop gap if more processes are needed. 
	
	//initialize original board on global root
	if(global_rank == 0){boardState = initalize_root_board(N, type_of_matrix);}
	//if(global_rank == 0){boardState = (char*) calloc((N+2) * (N+2), sizeof(char));}
    ...
    int color = 0; 
	//setup initial phase
    //printf("RANK %d at phase start\n", global_rank);
	//scatter initial grid state
	if(global_rank==0){printf("phase_index, phase_size, start_up, avg_calc_time, min_calc_time, halo_time, total_time \n");}
	for(int phase = 0; phase < num_phases; phase++)
	{
		local_calc_time =0;
		local_halo_time =0;
		setup_start = MPI_Wtime();
		color = setup_phase(&local, &local_new, N, phases[phase], global_rank, &phase_comm);
		setup_end = MPI_Wtime();
		...
		if(color == 1){
			...
			for (i = 0; i < nsteps; i++)
			{
				...
				//if(global_rank==0){printf("%d \n", i);}				
			}
			//if(global_rank==0){printf("at phase end\n");}
